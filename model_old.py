...
class TabNet(Model):
    def __init__(self, feature_columns, num_features, feature_dim, output_dim, num_decision_steps, relaxation_factor, virtual_batch_size,
        ...
        for step in range(self.num_decision_steps):
            ...
            step_layers.append(Dense(units= self.feature_dim * 2, name= f"transformer1_{str(step)}", use_bias= False))
            step_layers.append(BatchNormalization(momentum= self.batch_momentum, virtual_batch_size= self.v_b))
            #put glu layer
            step_layers.append(Dense(units= self.feature_dim * 2, name= f"transformer2_{str(step)}", use_bias= False))
            step_layers.append(BatchNormalization(momentum= self.batch_momentum, virtual_batch_size = self.v_b))
            #glu layer and previous residue
            step_layers.append(Dense(units= self.feature_dim * 2, name = f"transformer3_{step}", use_bias= False))
            step_layers.append(BatchNormalization(momentum= self.batch_momentum, virtual_batch_size = self.v_b))
            #glu layer and previous residue
            step_layers.append(Dense(units= self.feature_dim * 2, name= f"transformer4_{step}", use_bias= False))
            step_layers.append(BatchNormalization(momentum= self.batch_momentum, virtual_batch_size = self.v_b))
            self.transformer_layers.append(step_layers)
            #attentive_transformer layers
            attentive_step_layers.append(Dense(units = self.num_features, name = f"transform_coed_{str(step)}", use_bias = False))
